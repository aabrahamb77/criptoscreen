...
const state = {
  broker: null, stream: null, breaker: null,
  lastSignals: [], regimes: {}, btcRegime: null, heatmaps: {},
  started: false,
};

async function cycle(broker, stream, breaker) {
  ...
  for (const symbol of C.SYMBOLS) {
    try {
      ...
      const guard = correlationGuard([...broker.positions.values()], best, state.btcRegime);
      if (!guard.ok) { continue; }

      signals.push(best);
      const pos = broker.open(best);
      if (pos) {
        console.log(`[ABIERTA][${best.strategy}] ${best.side.toUpperCase()} ${symbol} @ ${pos.entry.toFixed(6)} ` +
          `| SL ${pos.stop.toFixed(6)} TP ${pos.takeProfit.toFixed(6)} | score ${best.score} | régimen ${regime.regime} | ${best.reason}`);
      }
    } catch (e) {
...
async function startBot() {
  ...
  state.broker = broker; state.stream = stream; state.breaker = breaker;
  ...
  const loop = async () => {
    const t0 = Date.now();
    try { await cycle(broker, stream, breaker); } catch (e) { console.warn('ciclo:', e.message); }
    const s = broker.stats(); const b = breaker.state();
    console.log(`[WS ${stream.connected ? 'ON' : 'OFF'}] equity=${s.equity} abiertas=${s.openPositions} cerradas=${s.trades || 0}` +
...
module.exports = { startBot, getState, suggestSize };
